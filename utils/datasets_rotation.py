...
def create_dataloader(path, imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False, cache=False, pad=0.0,
                      rect=False, rotation=True, rank=-1, world_size=1, workers=8, image_weights=False, quad=False,
                      prefix=''):
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, rotation=True,
                 image_weights=False,
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        with Pool(num_threads) as pool:
            pbar = tqdm(
                pool.imap_unordered(verify_image_label_rotation, zip(self.img_files, self.label_files, repeat(prefix))),
...
def create_dataloader_v2(path, imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False, cache=False, pad=0.0,
                      rect=False, rotation=True,rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix=''):
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache
    with torch_distributed_zero_first(rank):
        dataset = LoadImagesAndLabels_v2(path, imgsz, batch_size,
                                      augment=augment,  # augment images
                                      hyp=hyp,  # augmentation hyperparameters
                                      rect=rect,  # rectangular training
                                      rotation=True,
    ...
    nw = min([os.cpu_count() // world_size, batch_size if batch_size > 1 else 0, workers])  # number of workers
    sampler = torch.utils.data.distributed.DistributedSampler(dataset) if rank != -1 else None
    loader = torch.utils.data.DataLoader if image_weights else InfiniteDataLoader_v2
    # Use torch.utils.data.DataLoader() if dataset.properties will update during training else InfiniteDataLoader()
    dataloader = loader(dataset,
                        batch_size=batch_size,
                        num_workers=nw,
                        sampler=sampler,
                        pin_memory=True,
                        collate_fn=LoadImagesAndLabels_v2.collate_fn4 if quad else LoadImagesAndLabels_v2.collate_fn)
    return dataloader, dataset


class InfiniteDataLoader_v2(torch.utils.data.dataloader.DataLoader):
    """ Dataloader that reuses workers
...
class LoadImagesAndLabels_v2(Dataset):  # for training/testing
...
def random_perspective_rotation(img, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10,
                                perspective=0.0,
                                border=(0, 0)):
    # torchvision.transforms.RandomAffine(degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-10, 10))
    # targets = [cls, xyxy]
    ...
    if n:
        ...
        # filter candidates
        i = box_candidates_rotation(box1=targets[:, 1:5].T * s, box2=new.T, width=width, height=height,
...
def box_candidates_rotation(box1, box2, width, height, wh_thr=2, ar_thr=20, area_thr=0.1,
    ...
    return save_box & (w2 > wh_thr) & (h2 > wh_thr) & (w2 * h2 / (w1 * h1 + eps) > area_thr) & (
